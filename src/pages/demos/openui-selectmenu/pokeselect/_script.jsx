...
const App = () => {
  ...
  React.useEffect(() => {
    const grabTheOneFiftyOne = async () => {
      const result = await(await fetch('https://pokeapi.co/api/v2/pokemon?limit=151%27')).json()
  ...
  return (
    <>
      <selectmenu
        ...
        data-type={selected ? selected.types[0].type.name : null}
        ref={selectMenuRef}
        onInput={onInput}
      >
        <div slot="button">
          <button behavior="button" disabled={loading}>
            <span className="pokeselect__image-placeholder">
              {loading && <img className="pokeselect__ball" src="/shared/images/poke-ball.png" alt="" />}
              {!loading && !selected && <img className="pokeselect__choose" src="/shared/images/question.png" alt="" />} 
        ...
        <div slot="listbox">
          <div popover="auto" behavior="listbox">
            {options && Object.keys(options).length && Object.keys(options).map((group) => {
              return (
                <div key={`group--${group}`}label={`${group.charAt(0).toUpperCase()}${group.slice(1)}`}>
